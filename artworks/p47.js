...
function setupShapes() {
  ...
  // Assign a random shape to each position (all from the same term)
  shapeAssignments = new Map();
  for (let i = 0; i < allPositions.length; i++) {
    const pos = allPositions[i];
    const key = pos.type + '-' + pos.row + '-' + pos.col;
    const randomShape = random(filteredShapes);
    shapeAssignments.set(key, randomShape);
  }
...
function draw() {
  ...
  const shapeSize2x2 = min(cellWidth * 2, cellHeight * 2) * 0.85;
  
  // Draw grid cells - each cell gets a shape positioned at top-left
  for (let row = 0; row < rows; row++) {
    for (let col = 0; col < cols; col++) {
      ...
      if (isIn2x2Block(row, col)) {
        ...
        // Draw 1x1 cell shape at top-left of cell
  ...
  // Display the specific terms used
  fill(0);
  textAlign(CENTER, BOTTOM);
  textSize(14);
  text(usedTerms.join(', '), width / 2, height - 12);
}
